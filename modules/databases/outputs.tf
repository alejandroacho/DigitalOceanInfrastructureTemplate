...
output "production_mysql_port" {
  value = try(resource.digitalocean_database_cluster.production_mysql[0].port, "")
}

output "dev_staging_mysql_urn" {
  value = try(resource.digitalocean_database_cluster.dev_staging_mysql[0].urn, "")
}

output "dev_staging_redis_urn" {
  value = try(resource.digitalocean_database_cluster.dev_staging_redis[0].urn, "")
}

output "dev_staging_mysql_host" {
  value = try(resource.digitalocean_database_cluster.dev_staging_mysql[0].host, "")
}

output "dev_staging_mysql_port" {
  value = try(resource.digitalocean_database_cluster.dev_staging_mysql[0].port, "")
}
